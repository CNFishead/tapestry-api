...
export class ReceiptHandler extends CRUDHandler<ReceiptType> {
  ...
  async paymentStatistics(userId: string): Promise<any> {
    try {
      const receipts = await this.Schema.find({ userId: userId as any });
      const totalReceipts = receipts.length;
      // total amount paid from successful payments
      const totalAmountPaid = receipts.filter((r) => r.status === 'success' || r.status === 'succeeded').reduce((sum, r) => sum + r.amount, 0);
      // total refunds
