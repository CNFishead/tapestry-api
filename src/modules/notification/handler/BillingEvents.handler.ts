...
export default class BillingEventHandler {
  ...
  paymentSuccess = async (event: any) => {
    ...
    try {
      ...
      await Promise.all([
        this.insertPaymentSuccessNotification(user._id.toString(), receipt),
        this.sendPaymentSuccessEmail(user, receipt),
        this.sendPaymentSuccessSMS(user, receipt),
      ]);
  ...
  paymentFailed = async (event: any) => {
    ...
    try {
      ...
      // Send notifications in parallel
      await Promise.all([
        this.insertPaymentFailureNotification(user._id.toString(), receipt),
        this.sendPaymentFailureEmail(user, receipt),
        this.sendPaymentFailureSMS(user, receipt),
      ]);
